...
def fit_lstm(train, n_lag, n_seq, n_batch, nb_epoch, n_neurons1, n_neurons2):
...
def evaluate_rmse(test, forecasts, n_lag, n_seq, turn = 1):
	...
	for i in range(n_seq):
		...
		sum = sum + rmse
		rmse_per_turn.append(rmse)
	print('average: {0:.1f}, turn {1}'.format(sum/7, turn))
	return rmse_per_turn
...
def evaluate_mape(test, forecasts, n_lag, n_seq, turn = 1):
	...
	for i in range(n_seq):
		...
		sum = sum + mape
		mape_per_turn.append(mape)
	print('average: {0:.2f}, turn {1}'.format(sum/7, turn))
	return mape_per_turn
...
for i in a[::4]:
	for j in b[::4]:
'''
#for i,j in [(24,8), (24,12), (24,16), (12,4), (12, 12), (16,4)]:
	# load dataset
i=12
j=4
...
for r in range(repeats):
	# fit model
	model = fit_lstm(train, n_lag, n_seq, n_batch, n_epochs, n_neurons1, n_neurons2)
	# make forecasts
	...
	# evaluate forecasts
	rmse_list.append(evaluate_rmse(actual, forecasts, n_lag, n_seq, r+1))
	mape_list.append(evaluate_mape(actual, forecasts, n_lag, n_seq, r+1))

print(DataFrame(rmse_list).describe())
print(DataFrame(mape_list).describe())
pyplot.figure()
pyplot.boxplot(rmse_list)
pyplot.figure()
pyplot.boxplot(mape_list)
pyplot.show()
