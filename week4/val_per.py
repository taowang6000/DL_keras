...
def fit_lstm(train, n_lag, n_seq, n_batch, nb_epoch, n_val, n_neurons):
	...
	train_X, train_y = X[0:-n_val * 22, :, :], y[0:-n_val * 22, :]
	if n_val == 1:
		vali_X, vali_y = X[-22:, :, :], y[-22:, :]
	else:
		vali_X, vali_y = X[-n_val*22 : -(n_val-1)*22, :, :], y[-n_val*22 : -(n_val-1)*22, :]

	print("lenth of train_X: {0} lenth of vali_X {1}".format(len(train_X), len(vali_X)))
	...
	for j in range(nb_epoch):
		...
		model.reset_states()
	pyplot.plot(range(1, len(turn_mae) + 1), turn_mae)
