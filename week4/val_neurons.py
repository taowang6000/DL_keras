...
def fit_lstm(train, n_lag, n_seq, n_batch, nb_epoch, n_val, n_neurons):
...
def evaluate_rmse(test, forecasts, n_lag, n_seq, n_neurons):
	print ('RMSE: ', end = '')
	sum = 0
	for i in range(n_seq):
		...
		sum = sum + rmse
	print('average: {0:.1f}, neurons: {1}'.format(sum/7, n_neurons))

def evaluate_mape(test, forecasts, n_lag, n_seq, n_neurons):
	print ('MAPE: ', end = '')
	sum = 0
	for i in range(n_seq):
		actual = [row[i] for row in test]
		...
		sum = sum + mape
	print('average: {0:.2f}, neurons: {1}'.format(sum/7, n_neurons))
...
for j in range(10):
	...
	# load dataset
		...
		n_epochs = 12
		n_batch = 1
		n_val = [245, 267, 311, 399, 465]   # validation turns
		n_neurons = i + 1
		# prepare data
		scaler, train, test = prepare_data(series, n_test, n_lag, n_seq)
		# fit model
		mae = fit_lstm(train, n_lag, n_seq, n_batch, n_epochs, n_val, n_neurons)
		turn_mae.append(mae)
	all_mae_neu.append(turn_mae)
	print("turn {0}".format(j))
average_mae_neu = [mean([x[k] for x in all_mae_neu]) for k in range(40)]
for i in range(40):
	print('MAE: {0:.4f}, neurons: {1}'.format(average_mae_neu[i], i + 41))
pyplot.plot(range(1, len(average_mae_neu) + 1), average_mae_neu)
pyplot.xlabel('Neurons (+40)')
pyplot.ylabel('Validation Mean Absolute Error')
pyplot.show()		
