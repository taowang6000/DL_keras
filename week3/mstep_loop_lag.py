...
def evaluate_rmse(test, forecasts, n_lag, n_seq, turn = 1):
	...
	for i in range(n_seq):
		...
		sum = sum + rmse
	print('average: {0:.1f}, n_lag: {1}'.format(sum/n_seq, n_lag))
...
def evaluate_mape(test, forecasts, n_lag, n_seq, turn = 1):
	...
	for i in range(n_seq):
		...
		sum = sum + mape
	print('average: {0:.2f}, n_lag: {1}'.format(sum/n_seq, n_lag))
...
import numpy
a = numpy.random.randint(10000, size=(1, 20))[0].tolist()
b = numpy.random.randint(10000, size=(1, 20))[0].tolist()
c = numpy.random.randint(10000, size=(1, 20))[0].tolist()
for j in range(20):
	for i in range(20):
		...
		numpy.random.seed(a[j])
		rn.seed(b[j])
		session_conf = tf.ConfigProto(intra_op_parallelism_threads=1, inter_op_parallelism_threads=1)

		from keras import backend as K
		tf.set_random_seed(c[j])
